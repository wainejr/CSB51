...
class TopicsOverTime:
    def GetPnasCorpusAndDictionary(
        self, documents_path, timestamps_path, stopwords_path
    ):
        documents = []
        timestamps = []
        dictionary = set()
        ...
        for doc in fileinput.input(documents_path):
            words = [
                word for word in doc.lower().strip().split() if word not in stopwords
            ]
        ...
        timestamps = [
            1.0 * (t - first_timestamp) / (last_timestamp - first_timestamp)
            for t in timestamps
        ]
    ...
    def InitializeParameters(self, documents, timestamps, dictionary):
        ...
        par["max_iterations"] = 60  # max number of iterations in gibbs sampling
        ...
        par["betafunc_psi"] = [
            scipy.special.beta(par["psi"][t][0], par["psi"][t][1])
            for t in range(par["T"])
        ]
        par["word_id"] = {dictionary[i]: i for i in range(len(dictionary))}
        par["word_token"] = dictionary
        par["z"] = [
            [random.randrange(0, par["T"]) for _ in range(par["N"][d])]
            for d in range(par["D"])
        ]
        par["t"] = [
            [timestamps[d] for _ in range(par["N"][d])] for d in range(par["D"])
        ]
        par["w"] = [
            [par["word_id"][documents[d][i]] for i in range(par["N"][d])]
            for d in range(par["D"])
        ]
    ...
    def GetTopicTimestamps(self, par):
        ...
        for topic in range(par["T"]):
            ...
            current_topic_doc_timestamps = [
                [(par["z"][d][i] == topic) * par["t"][d][i] for i in range(par["N"][d])]
                for d in range(par["D"])
            ]
            for d in range(par["D"]):
                ...
                for i in range(par["N"][d]):
                    ...
                    for topic_di in range(par["T"]):
                        ...
                        topic_probability = 1.0 * (
                            par["m"][d][topic_di] + par["alpha"][topic_di]
                        )
                        ...
                        topic_probability *= (
                            par["n"][topic_di][word_di] + par["beta"][word_di]
                        )
                    ...
                    topic_probabilities = (
                        pd.Series(topic_probabilities).fillna(0).tolist()
                    )
                    new_topic = list(
